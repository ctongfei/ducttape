...
object BranchPointHandler extends Logging {
    ...
    (implicit taskTemplateBuilder: TaskTemplateBuilder)
{
    ...
    curSpec.rValue match {
      ...
      case ConfigVariable(varName) => {
        taskTemplateBuilder.confSpecs.get(varName) match {
          case Some(confSpec) => {
            ...
            branchSpec.rValue match {
              // we found a nested branch point -- recursively call resolveBranchPoint()
                case BranchPointDef(_,_) => {
                  ...
                  val bpTreeData: BranchPointTreeGrafts = prevTree.getOrAdd(branchPoint, prevGrafts)
